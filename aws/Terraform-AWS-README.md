...
6. 「aws」フォルダをダブルクリックし、「Terraform-AWS-README.md」をクリックして画面中央部分に表示します。「Terraform-AWS-README.md」はAWS LightsailでTerraformを使う際の手順がかかれているファイルです。「.md（えむでぃー）」形式のファイルが読みにくい場合は、Visual Studio Code拡張機能の「Markdown Preview Enhanced」を使うと読みやすくなります。

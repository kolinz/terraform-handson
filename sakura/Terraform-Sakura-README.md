...
6. 「sakura」フォルダをダブルクリックし、「Terraform-Sakura-README.md」をクリックして画面中央部分に表示します。「Terraform-Sakura-README.md」は、さくらのクラウドで、Terraformを使う際の手順がかかれているファイルです。「.md（えむでぃー）」形式のファイルが読みにくい場合は、Visual Studio Code拡張機能の「Markdown Preview Enhanced」を使うと読みやすくなります。
...
1. Visual Studio Codeで、画面上部のメニューバーで、「ターミナル」>>「新しいターミナル」の順にクリックします。画面上部に「ターミナル」が表示されない場合は、「実行」の右隣の「・・・」をクリックしてください。「ターミナル」が表示されます。
...
Terraformにスタートアップスクリプトを組み込むことで、VM作成・起動時にソフトウェアのインストールを自動で行うようなことができます。
